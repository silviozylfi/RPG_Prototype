...
namespace RPG.Shops
{
    public class Shop : MonoBehaviour, IRaycastable, ISaveable
    {
        ...
        public IEnumerable<ShopItem> GetFilteredItems()
        {
            ...
            {
                if(filter == ItemCategory.None || item.GetInventoryItem().GetItemCategory() == filter)
                {
                    yield return item;
                }
            }
        ...
        public bool HasSufficientFunds()
        {
            ...
            return TransactionTotal() <= currentShopper.GetComponent<Purse>().GetBalance();
        }
        ...
        {
            Dictionary<string, int> savedObject = new Dictionary<string, int>();
            foreach(var pair in stockSold)
            {
                savedObject[pair.Key.GetItemID()] = pair.Value;
            }
        ...
        {
            Dictionary<string, int> savedObject = (Dictionary<string, int>)state;
            stockSold.Clear();
            foreach(var pair in savedObject)
            {
